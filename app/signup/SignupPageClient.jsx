...
export default function SignupPage() {
	...
	const handleOtpSubmit = async (event) => {
		...
		try {
			const result = await verifyOtp({ email, code: otp, action: "signup" });
			if (result.success) {
				router.push("/dashboard");
			} else {
				...
				<div className='rounded-2xl bg-white p-8 shadow-xl'>
					<form
						...
						{step === "credentials" && (
							<>
								<div>
									<label htmlFor='name' className='font-montserrat mb-2 block text-sm font-semibold text-gray-700'>
										Full Name
									</label>
									<input
										id='name'
								...
								<p className='font-montserrat text-xs text-gray-500'>We will send a verification code to your email to complete the signup.</p>
							</>
						)}
						...
						{step === "otp" && (
							<div>
								...
								/>
								<p className='font-montserrat mt-2 text-xs text-gray-500'>Check your inbox for the security code we sent to {email}.</p>
							</div>
						)}
